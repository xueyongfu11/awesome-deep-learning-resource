...
### 多个 CUDA stream 是否都能异步执行？当每个 stream 依次包含 H2D、kernel 和 D2H 时，整体执行顺序是什么？

CUDA stream 可以作为相对于 CPU 异步提交工作的队列，但“异步提交”不等于不同 stream 一定并发执行。CUDA 的核心顺序保证是：**同一 stream 内按提交顺序执行，不同 stream 之间默认没有固定的执行顺序。**

假设每个 stream 操作的数据相互独立，并依次提交：

```text
Stream i: H2D_i -> Kernel_i -> D2H_i
```

那么 CUDA 必须满足每条 stream 内的依赖：`H2D_i` 完成后才能执行 `Kernel_i`，`Kernel_i` 完成后才能执行 `D2H_i`；但不同 stream 之间可以采用任意满足这些依赖的调度顺序。例如，既可能完全串行，也可能形成以下流水线：

| 执行资源 | 阶段 1 | 阶段 2 | 阶段 3 | 阶段 4 | 阶段 5 |
| --- | --- | --- | --- | --- | --- |
| H2D copy engine | H2D_1 | H2D_2 | H2D_3 |  |  |
| GPU 计算资源 |  | Kernel_1 | Kernel_2 | Kernel_3 |  |
| D2H copy engine |  |  | D2H_1 | D2H_2 | D2H_3 |

因此，另一个 stream 的 H2D、kernel 或 D2H 可能与当前 stream 的操作重叠；不同 kernel 在资源充足且设备支持 concurrent kernel execution 时也可能并发。但这些只是允许发生的情况，并不是 CUDA API 的并发保证。实际执行还受以下条件限制：

- H2D/D2H 与 kernel 要真正重叠，host buffer 通常需要使用 pinned memory；涉及 pageable host memory 的异步拷贝可能无法获得预期的异步和重叠效果。
- H2D 与 D2H 能否同时执行取决于 GPU 的异步 copy engine 数量和方向支持；只有一个 copy engine 时，拷贝通常需要串行。
- kernel 是否并发取决于 SM、寄存器、shared memory、block slots 等资源；一个 kernel 占满资源时，其他 stream 的 kernel 仍需等待。
- event、显式同步、数据依赖以及 legacy default stream 的隐式同步都会限制并发；可使用 per-thread default stream 或按需创建 `cudaStreamNonBlocking` stream。
- CPU 在读取 D2H 目标数据前，必须通过 stream 同步、event 或其他正确的同步机制确认拷贝已经完成。

总结：多个 stream 都可以承载异步工作，但整体执行顺序只是所有 `H2D_i -> Kernel_i -> D2H_i` 依赖链的一个合法交错；是否真正形成拷贝与计算流水线，需要结合硬件能力、内存类型、资源占用和同步关系判断。
