...
- LESS: Selecting Influential Data for Targeted Instruction Tuning
  ...
  - 为单个训练数据点计算Adam LoRA梯度特征，并将其保存在梯度数据存储库中。
  - 选择数据。对于具有少量示例的任务（含多个子任务），研究人员计算每个验证子任务的梯度特征。再从存储库中选出排名前5%训练子集。
  - 训练目标模型。该模型可以使用 LoRA 或完全微调进行训练。
...
- Lion: Adversarial Distillation of Closed-Source Large Language Model
  - 框架通过多轮迭代（实验中设为 3 轮）持续优化学生模型，每轮迭代包含 “模仿 - 辨别 - 生成” 3 个阶段，形成 “反馈闭环”
...
- LIMA: Less Is More for Alignment
  ...
  - 这篇论文如何提升数据质量？多样性：覆盖论坛的多个学科类别，质量：使用评分高的问题，然后再获取评分高的回复，过滤和清洗，添加部分高质量人工构造数据。
