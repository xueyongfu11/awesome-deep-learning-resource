...
- Knowledge-Instruct: Effective Continual Pre-training from Limited Data using Instructions
  - 2025.04，Microsoft Industry AI
  - 论文提出 Knowledge-Instruct：不要直接拿原始文档做 CPT，而是先把小语料转成高密度事实指令数据，再做 instruction tuning。
    - 先让一个外部 LLM 从每篇文档中识别实体。对每个实体，模型继续从文档中抽取相关事实。这个过程会重复多轮：第一轮抽取事实，后面几轮让模型检查还有哪些事实漏掉了，以提高覆盖率。
    - 抽出来的事实可能依赖上下文，会把事实改写成上下文完整的形式，让实体明确出现在事实中。
    - 系统会去掉重复事实，然后为每条事实生成多个 paraphrase。论文强调，事实的多种表达有助于模型更稳定地学习知识；实验也发现，paraphrase 数量越多效果越好，大约到 3 个左右开始趋于稳定，5 个已经能带来明显收益。
    - 把事实改成 instruction-response 对，进行指令微调。
  - ![image-20260508172124293](../assets/know_instruct.png)
- Injecting New Knowledge into Large Language Models via Supervised Fine-Tuning
  ...
  - 作者对同一批文档生成两类 Q&A 数据集，并做 1x/5x/10x 的规模扩展（scaling）实验，核心差别在你扩展的是 tokens 还是 facts。
    ...
    - Fact-based scaling：先让 GPT-4 把文档拆成一条条 atomic facts（原子事实），然后对每条事实生成多组（例如 10 组）不同问法的 Q&A，保证每个事实都有训练样本覆盖；同样再抽子集得到 1x/5x/10x。每个事实都有样本，扩展时是在增加同一事实的多样问法/表达，所以覆盖更系统、更均匀。具体是：让 GPT-4 把每段文章拆成尽可能简单的事实；如果一句话包含多个事实，就继续拆；同时要求避免代词，用完整名称代替 “it/he/this”等
  - 他们微调的是 GPT-4 v0613（文中提到其截止日期为 2021 年 9 月）
  - 实验结果，在Fact-based数据上训练的模型相比在Token-based数据上效果更好；Token-based：能涨分，但覆盖不均、后期收益变差甚至退化；Fact-based：扩展更稳定，没有规模越大反而退化的问题。
  - ![image-20260508162357002](../assets/factor_based.png)
  - ![image-20260508162555073](../assets/token_based.png)